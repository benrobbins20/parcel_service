...
def main():
    # instances of DataSeries with a numpy object for: 
    #   distance table: raw lookup table with the distances between locations
    #   package table: all 40 packages with:
    #       id, address, city, state, zip, deadline, weight, note
    #   address table: 26 addresses with id, location, address
    ds = DataSeries()
    ...
    # loop through package table (skip header row) and insert into hash table
    ...
    truck1 = TruckObject(truck_num=1, payload_capacity=1000, packages=truck1_packages, mileage_sum=0, cur_addr=hub)
